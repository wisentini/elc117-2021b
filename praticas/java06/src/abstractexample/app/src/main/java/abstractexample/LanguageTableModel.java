...
class LanguageTableModel extends AbstractTableModel {
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        ...
        switch (columnIndex) {
            case 0:
                return row.getLanguageId();
            case 1:
                return row.getFirstAppeared();
            case 2:
                return row.getParadigm();
            case 3:
    ...
    public void setValueAt(Object value, int rowIndex, int columnIndex) {
        ...
        switch (columnIndex) {
            case 0:
                row.setLanguageId((String) value);
                break;
            case 1:
                row.setFirstAppeared((String) value);
                break;
            case 2:
                row.setParadigm((String) value);
                break;
            case 3:
                row.setUserId((String) value);
                break;
        }
    }
    ...
    public String languagesToJSON(List<Language> languages) {
        String content = "[\n";

        for (Language language : languages) {
            content += language.toString();

            if (languages.indexOf(language) + 1 < languages.size()) {
                content += ",";
            }
    ...
    public void writeLanguagesToFile(List<Language> languages, String fileName) {
        try {
            FileWriter fileWriter = new FileWriter(fileName);
            String content = languagesToJSON(languages);
